...
function SearchResults() {
  ...
  const filteredSearchedData = data ? data.filter(createFilter(searchTermNormalized, KEYS_TO_FILTERS)) : [];
  ...
  const listsContainingSearchTerm = filteredSearchedData.filter(entry => {
    return entry.list && entry.list.toLowerCase().includes(searchTermNormalized);
  });
      ...
      {isLoading ? (
        ...
        <>
          {listsContainingSearchTerm.length > 0 ? (
            ...
            <div className={styles.searchSummary}>
Event found! Here are the Top {filteredSearchedData.length} Players!
            </div>
          )}
